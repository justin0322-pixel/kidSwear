...
describe('ShopsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [ShopsService, { provide: PrismaService, useValue: p }],
    }).compile();
  ...
  describe('removeVipMember()', () => {
    ...
    it('成員不存在時拋出 NotFoundException', async () => {
      p.wholesaler.findUnique.mockResolvedValue(makeWholesaler());
      p.shopVipMember.findUnique.mockResolvedValue(null);
      await expect(service.removeVipMember(BigInt(10), BigInt(5))).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('setVipDiscount()', () => {
    ...
    it('正常設定百分比折扣（upsert）', async () => {
      ...
      const result = await service.setVipDiscount(userId, variantId, {
        discountType: DiscountType.percentage,
        discountValue: 10,
      });
    ...
    it('規格不屬於此商城時拋出 NotFoundException', async () => {
      ...
      p.productVariant.findFirst.mockResolvedValue(
        makeVariant({ product: { shopId: BigInt(99), name: 'X' } }),
      );
      await expect(
        service.setVipDiscount(userId, variantId, {
          discountType: DiscountType.percentage,
          discountValue: 10,
        }),
  ...
  describe('removeVipDiscount()', () => {
    ...
    it('折扣不存在時拋出 NotFoundException', async () => {
      ...
      await expect(service.removeVipDiscount(BigInt(10), BigInt(100))).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateMyShop()', () => {
    ...
    it('非批發商時拋出 ForbiddenException', async () => {
      p.wholesaler.findUnique.mockResolvedValue(null);
      await expect(service.updateMyShop(BigInt(99), { name: '新名稱' })).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
