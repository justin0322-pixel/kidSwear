...
describe('CartService', () => {
  ...
  let redis: { hgetall: jest.Mock; hget: jest.Mock; hset: jest.Mock; expire: jest.Mock; hexists: jest.Mock; hdel: jest.Mock; del: jest.Mock }
  ...
  describe('addItem()', () => {
    it('variant 不存在時拋出 NotFoundException', async () => {
      ...
      await expect(service.addItem(userId, { variantId: 999, quantity: 1 })).rejects.toThrow(NotFoundException)
    })
