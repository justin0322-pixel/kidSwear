...
export class CartService {
  ...
  async addItem(userId: bigint, dto: AddCartItemDto) {
    ...
    if (!variant)
      throw new NotFoundException({ code: 'RESOURCE_NOT_FOUND', message: '商品規格不存在' });
  ...
  async updateItem(userId: bigint, variantId: number, quantity: number) {
    ...
    if (!exists)
      throw new NotFoundException({ code: 'RESOURCE_NOT_FOUND', message: '購物車中無此商品' });
