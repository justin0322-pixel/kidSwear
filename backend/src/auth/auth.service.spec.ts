...
describe('AuthService', () => {
  ...
  describe('register()', () => {
    ...
    it('批發商正常註冊', async () => {
      ...
      const user = makeUser({
        ...
        wholesaler: { companyName: 'ABC', contactPerson: '老闆' },
      });
      p.user.create.mockResolvedValue(user);
  ...
  describe('login()', () => {
    ...
    it('帳號停用時拋出 UnauthorizedException', async () => {
      ...
      usersSvc.findByEmail.mockResolvedValue(
        makeUser({ passwordHash: hash, status: UserStatus.suspended }),
      );
      await expect(service.login(dto, res as never)).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('changePassword()', () => {
    it('正確舊密碼時成功更新', async () => {
      ...
      await expect(
        service.changePassword(BigInt(1), 'oldpass', 'newpass123'),
    ...
    it('舊密碼錯誤時拋出 BadRequestException', async () => {
      ...
      await expect(service.changePassword(BigInt(1), 'wrongpass', 'newpass123')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('OAuth 帳號（無 passwordHash）拋出 BadRequestException', async () => {
      ...
      await expect(service.changePassword(BigInt(1), 'anypass', 'newpass123')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
