...
const makeProduct = (overrides = {}) => ({
  ...
  description: '好看的上衣', skuPrefix: 'BEAR',
  ageRange: '1-2歲', gender: 'unisex',
  basePrice: D('250'), suggestedRetailPrice: null,
  attributes: {}, status: ProductStatus.active,
  deletedAt: null, createdAt: new Date(),
  shopId: BigInt(2),
  images: [{ id: BigInt(1), url: 'https://example.com/img.jpg', isPrimary: true, altText: null, sortOrder: 0 }],
  variants: [{ id: BigInt(100), sku: 'BEAR-90-PINK-0', size: '90cm', color: '粉紅', stock: 10, priceOverride: null }],
...
describe('ProductsService', () => {
  ...
  describe('findById()', () => {
    ...
    it('variant 有 priceOverride 時以覆蓋價格回傳', async () => {
      p.product.findFirst.mockResolvedValue(
        makeProduct({
          variants: [{ id: BigInt(100), sku: 'BEAR-90-PINK-0', size: '90cm', color: '粉紅', stock: 10, priceOverride: D('199') }],
        }),
      )
      const result = await service.findById(BigInt(10))
  ...
  describe('create()', () => {
    ...
    const dto: CreateProductDto = {
      name: '小熊上衣', category: '上衣', basePrice: '250',
  ...
  describe('update()', () => {
    ...
    it('非擁有者（商品屬於其他商城）拋出 NotFoundException', async () => {
      ...
      await expect(service.update(userId, productId, { name: '新名稱' })).rejects.toThrow(NotFoundException)
    })
